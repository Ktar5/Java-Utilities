...
public abstract class GameStateMachine<S, T> extends StateMachine<S, T> {
    ...
    public TriggerWithParameters<S, T> getTrigParam(T trigger) {
        return this.configuration().getTriggerConfiguration(trigger);
    }

    protected <A1> TriggerWithParameters1<A1, S, T> onEntryFromSelf(S state, Class<A1> arg1, Action1<A1> action) {
        ...
        TriggerWithParameters1<A1, S, T> triggerParameters
    ...
    protected <A1, A2> TriggerWithParameters2<A1, A2, S, T> onEntryFromSelf(S state, Class<A1> arg1, Class<A2> arg2, Action2<A1, A2> action) {
        if (this.configuration().getRepresentation(state) == null) {
            this.configuration().configure(state);
        }
        if (this.configuration().isTriggerConfigured(getTriggerFrom(state))) {
        ...
        TriggerWithParameters2<A1, A2, S, T> triggerParameters
                = this.configuration().setTriggerParameters(getTriggerFrom(state), arg1, arg2);
        this.configuration().configure(state).onEntryFrom(triggerParameters, action);
        return triggerParameters;
    }

    protected <A1, A2, A3> TriggerWithParameters3<A1, A2, A3, S, T> onEntryFromSelf(S state, Class<A1> arg1, Class<A2> arg2, Class<A3> arg3, Action3<A1, A2, A3> action) {
        if (this.configuration().getRepresentation(state) == null) {
            this.configuration().configure(state);
        }
        if (this.configuration().isTriggerConfigured(getTriggerFrom(state))) {
        ...
        TriggerWithParameters3<A1, A2, A3, S, T> triggerParameters
